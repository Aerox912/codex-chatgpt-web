...
export async function ensureChatGptPersonalizedConnectorAccess(
  ...
  const unpersonalized = page
    .getByRole("button", { name: "Unpersonalized", exact: true })
    .filter({ visible: true });
  let personalizedCount = await personalized.count();
  let unpersonalizedCount = await unpersonalized.count();
...
export class ChatGptBrowserWorker {
      ...
      if (externalProgress) {
        ...
        try {
          const observed = await withBrowserTurnAbort(Promise.race([
            this.currentSubmissionEvidence(page, baseline).then(value => ({ kind: "dom" as const, value })),
        ...
        evidence = await this.currentSubmissionEvidence(page, baseline);
      }
  ...
  private async attachPrompt(
    ...
    if (!forceInline && chatGptPromptTransport(composerPrompt) === "native-paste-attachment") {
      const composer = await this.activeComposer(page);
      // ChatGPT's native large-paste behavior is tied to one complete editor fill. The
      // plain-text editing command deliberately bypasses it, so use one complete fill for ordinary
      // oversized prompts. Multipart protocol messages are forced inline by their caller.
  ...
  private async waitForMultipartAcknowledgement(
    ...
    for (;;) {
      ...
      if (!snapshot.responsePresent && await responseTurn.locator.count() !== 1) {
        const rebound = await this.reconcileAssistantTurnBinding(page, submissionBaseline, responseTurn);
  ...
  private async runBrowserTurn(
    ...
    try {
      ...
      const promptTransport: ChatGptPromptTransport = prepared.multipart
        ? "inline"
        : chatGptPromptTransport(prepared.text);
      ...
      console.info(
        `[chatgpt-web] browser turn ${turn.traceId} opened (transport=${prepared.multipart ? `multipart-${prepared.multipart.parts.length}` : promptTransport}, maxMessageChars=${maxMessageChars}, estimatedInputTokens=${estimatedInputTokens}, images=${prepared.images.length}, compactionTrimmedMessages=${prepared.trimmedCompactionMessages ?? 0})`,
      );
      ...
      for (;;) {
        ...
        await throwIfChatGptSessionFailureAlert(page);
        if (await chatGptTerminalErrorAlert(responseTurn.locator).isVisible().catch(() => false)) {
          ...
          if (!emittedVisibleText && retryDelay !== undefined) {
            ...
            if (await retryChatGptTerminalErrorAlert(responseTurn.locator)) {
        ...
        if (!snapshot.responsePresent) {
          try {
            const rebound = await withChatGptBrowserObservationTimeout(
              this.reconcileAssistantTurnBinding(page, submissionBaseline, responseTurn),
            );
            if (rebound.identity !== responseTurn.identity) {
              responseTurn = rebound;
              responseDomCache.key = undefined;
