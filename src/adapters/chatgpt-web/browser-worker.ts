...
type ChatGptTextScope = Pick<Locator, "getByRole" | "getByText" | "getByTestId">;
...
export class ChatGptBrowserWorker {
  ...
  private async attachPrompt(
    ...
    if (!forceInline && chatGptPromptTransport(composerPrompt) === "native-paste-attachment") {
      ...
      if (requireThink) {
        await setChatGptThinkMode(routedComposer.locator("xpath=ancestor::form[1]"), true, captureDiagnostic, abortSignal);
      }
