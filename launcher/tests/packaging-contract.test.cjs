...
test("the public launcher command uses the Electron bootstrap", () => {
  ...
  assert.equal(repositoryManifest.repository.url, "git+https://github.com/Aerox912/codex-chatgpt-web.git");
  assert.equal(repositoryManifest.homepage, "https://github.com/Aerox912/codex-chatgpt-web#readme");
});
...
test("release installers resolve checksummed native launcher assets", () => {
  ...
  const runtimeInstaller = fs.readFileSync(path.join(repositoryRoot, "scripts", "install.sh"), "utf8");
  ...
  for (const installer of [shellInstaller, windowsInstaller, runtimeInstaller]) {
    assert.match(installer, /Aerox912\/codex-chatgpt-web/);
    assert.doesNotMatch(installer, /miuuyy\/codex-chatgpt-web/);
  }
...
test("packaged launcher owns a detached checksummed updater for every release platform", () => {
  ...
  assert.match(updater, /Aerox912\/codex-chatgpt-web/);
  assert.match(main, /https:\/\/github\.com\/Aerox912\/codex-chatgpt-web/);
  assert.doesNotMatch(`${updater}\n${main}`, /miuuyy\/codex-chatgpt-web/);
...
test("CI validates native launcher packages without installing the Windows artifact", () => {
  ...
  assert.doesNotMatch(release, /cp scripts\/install\.sh/);
...
test("Windows releases embed the hash-pinned stable Bun runtime with the streaming fix", () => {
  ...
  const forkRuntime = fs.readFileSync(
    path.join(repositoryRoot, "scripts", "prepare-fork-bun-runtime.ps1"),
    "utf8",
  );
