...
import {
  ...
  chatGptPromptTransport,
  retryChatGptTerminalErrorAlert,
...
test("browser turn orchestration retains owned prompt insertion and semantic submission", () => {
  ...
  expect(runBrowserTurn).toContain("prepared.multipart !== undefined");
  expect(runBrowserTurn).toMatch(/stage\.text,[\s\S]*?undefined,[\s\S]*?true,[\s\S]*?\),/);
...
test("inline prompt attachment canonicalizes Windows and legacy Mac line endings", async () => {
  const prompt = "first\r\nsecond\rthird";
  ...
  await attachPrompt.call({
    activeComposer: async () => composer,
    insertPromptText: async (_page: unknown, value: string) => { inserted = value; },
    assertPromptAttached: async (_page: unknown, value: string) => { asserted = value; },
  }, dialogPage("").page, prompt, false);

  expect(inserted).toBe(canonical);
  expect(asserted).toBe(canonical);
  expect(chatGptPromptTransport(prompt)).toBe("inline");
});

test("Bigger Context protocol messages stay inline above the native-paste threshold", async () => {
  const prompt = [
    "CODEX_MULTIPART_STAGE 1/3",
    "```json",
    JSON.stringify({ payload: "x".repeat(220_000) }),
    "```",
  ].join("\n");
  const calls: Array<[string, unknown?]> = [];
  let asserted = "";
  const composer = {
    fill: async (value: string) => { calls.push(["fill", value]); },
    focus: async () => { calls.push(["focus"]); },
    evaluate: async (_fn: unknown, value: string, options: unknown) => {
      calls.push(["evaluate", value]);
      calls.push(["evaluateOptions", options]);
      return true;
    },
  ...
  const attachPrompt = (ChatGptBrowserWorker.prototype as unknown as {
    attachPrompt(
      ...
      connectorAttemptBudget?: undefined,
      forceInline?: boolean,
    ): Promise<void>;
  }).attachPrompt;
  const insertPromptText = (ChatGptBrowserWorker.prototype as unknown as {
  ...
  await attachPrompt.call({
    ...
    insertPromptText,
    assertPromptAttached: async (_page: unknown, value: string) => { asserted = value; },
  }, dialogPage("").page, prompt, false, undefined, undefined, false, undefined, true);
...
test("Bigger Context preflight does not reserve a native-paste attachment slot", () => {
  const workerSource = readFileSync(new URL("../src/adapters/chatgpt-web/browser-worker.ts", import.meta.url), "utf8");
  expect(workerSource).toMatch(
    /const promptTransport: ChatGptPromptTransport = prepared\.multipart\s*\? "inline"\s*: chatGptPromptTransport\(prepared\.text\)/,
  );
});
...
test("an oversized prompt uses ChatGPT's native pasted-text attachment before a small inline instruction", async () => {
  ...
  await attachPrompt.call({
    ...
    waitForNativePasteAttachmentReady,
    insertPromptText: async (_page: unknown, value: string) => {
...
test("an oversized Full-mode prompt preserves its native attachment while selecting the connector", async () => {
  const prompt = "x".repeat(CHATGPT_NATIVE_PASTE_ATTACHMENT_THRESHOLD_CHARS + 1);
  const calls: Array<[string, string?]> = [];
  const composer = {
    fill: async (value: string) => { calls.push(["fill", value]); },
  };
  const routedComposer = {
    focus: async () => { calls.push(["focus"]); },
  };
  const page = {
    ...dialogPage("").page,
  ...
  await attachPrompt.call({
    activeComposer: async () => composer,
    waitForNativePasteAttachmentReady: async () => { readinessChecks += 1; },
    selectConnector: async () => {
      calls.push(["selectConnector"]);
      return routedComposer;
    },
    insertPromptText: async (_page: unknown, value: string) => { calls.push(["insertText", value]); },
    assertPromptAttached: async (_page: unknown, value: string) => {
      expect(value).toBe(CHATGPT_NATIVE_PASTE_ATTACHMENT_INSTRUCTION);
    },
  }, page, prompt, true);
...
test("native prompt attachment conversion fails with an integrity error", async () => {
  const waitForNativePasteAttachmentReady = (ChatGptBrowserWorker.prototype as unknown as {
    waitForNativePasteAttachmentReady(
      page: unknown,
      prompt: string,
  ...
  await expect(waitForNativePasteAttachmentReady.call({}, {}, "oversized prompt", undefined, 0))
    .rejects.toBeInstanceOf(ChatGptPromptAttachmentIntegrityError);
});
...
test("retained tool turns insert into the connector-bound composer without selecting it again", async () => {
  ...
  }, dialogPage("").page, "retained context", true, undefined, undefined, false, undefined, false, true);
...
test("Think attachment runs after fresh connector selection and rechecks retained and Browser-only turns", async () => {
  ...
  for (const [localTools, retained] of [[true, false], [true, true], [false, false]]) {
    ...
    await attach.call(worker, ui.page, "requested task", localTools, undefined, undefined, false, undefined, false, retained, true);
    ...
    if (retained) {
      ...
      await attach.call(worker, ui.page, "follow-up task", localTools, undefined, undefined, false, undefined, false, retained, true);
...
test("Think mode is verified for native pasted-text attachments before inserting instructions", async () => {
  const attach = (ChatGptBrowserWorker.prototype as unknown as { attachPrompt: (...args: unknown[]) => Promise<void> }).attachPrompt;
  const ui = thinkSlashFixture();
  const fillDraft = ui.composer.fill;
  ui.composer.fill = async (text: string) => {
    // Native large-paste conversion moves the text into an attachment and clears the draft.
    await fillDraft(text.length > CHATGPT_NATIVE_PASTE_ATTACHMENT_THRESHOLD_CHARS ? "" : text);
  };
  const submitted: boolean[] = [];
  let readinessChecks = 0;
  const worker = {
  ...
  await attach.call(worker, ui.page, "x".repeat(CHATGPT_NATIVE_PASTE_ATTACHMENT_THRESHOLD_CHARS + 1), false,
    undefined, undefined, false, undefined, false, false, true);
  expect(submitted).toEqual([true]);
  expect(readinessChecks).toBe(2);
});
...
test("Think attachment rolls back a lost connector and never inserts the prompt", async () => {
  ...
  const attach = (ChatGptBrowserWorker.prototype as unknown as { attachPrompt: (...args: unknown[]) => Promise<void> }).attachPrompt;
  await expect(attach.call(worker, ui.page, "must not be inserted", true, undefined, undefined, false, undefined, false, false, true))
...
function dialogPage(text: string, buttonText = "Got it", errorActionVisible = false): { page: Page; pressed: string[] } {
  ...
  return {
    page: {
      ...
      getByText: (hasText: string | RegExp) => {
        activeDialog = createDialog();
        return activeDialog.filter({ hasText });
      },
      getByRole: (role: string, options?: { name?: string | RegExp }) => (
        activeDialog.getByRole(role, options)
      ),
      getByTestId: (testId: string) => {
        const action = {
          last: () => action,
...
test("the known terminal ChatGPT error can retry inside the retained browser turn", async () => {
  ...
  expect(await retryChatGptTerminalErrorAlert(fixture.page, 0)).toBeTrue();
  expect(fixture.pressed).toEqual(["Enter"]);
});
...
test("terminal model errors are scoped to the new assistant turn instead of global page alerts", () => {
  ...
  const recovery = workerSource.indexOf("retryChatGptTerminalErrorAlert(responseTurn.locator)");
  const failure = workerSource.lastIndexOf("throwIfChatGptTerminalErrorAlert(responseTurn.locator)");
  expect(recovery).toBeGreaterThan(-1);
  expect(failure).toBeGreaterThan(recovery);
  const retryBody = workerSource.slice(recovery, failure);
  expect(retryBody).toContain("completionTracker = new ChatGptCompletionTracker()");
  expect(retryBody).toContain("domHealthTracker = new ChatGptTurnDomHealthTracker()");
});
